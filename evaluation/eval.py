...
def main():
    ...
    if selected_file:
        ...
        with col2:
            ...
            filtered_data = data if selected_llm == "All" else [entry for entry in data if entry['llm_type'] == selected_llm]
            ...
            if filtered_data:
                ...
                with col_next:
                    if st.button("Next", disabled=st.session_state.current_index == len(filtered_data) - 1):
                        st.session_state.current_index = min(len(filtered_data) - 1, st.session_state.current_index + 1)
                        st.rerun()
                
                # Progress indicator
                st.write(f"Entry {st.session_state.current_index + 1} of {len(filtered_data)}")
            else:
