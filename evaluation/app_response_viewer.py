...
# Set page config
st.set_page_config(page_title="Chatbot Response Viewer", page_icon="🤖", layout="wide")
...
def load_all_data():
    ...
    for file_path in result_files:
        model_name = (
            os.path.basename(file_path).replace("results_", "").replace(".json", "")
        )
...
with st.sidebar:
    ...
    with nav_col2:
        if st.button(
            "Next ➡️", disabled=st.session_state.current_index == len(data) - 1
        ):
            next_item()
            st.rerun()
