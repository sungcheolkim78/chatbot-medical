...
class DatasetGenerator:
    def __init__(self, paper_path: str):
        ...
        self.document_loader = DocumentLoader(paper_path)
    ...
    def save_qa_pairs(self, qa_pairs: list, llm_type: LLMType, difficulty: Difficulty) -> None:
        ...
        try:
            ...
            logging.info(f"Saved {len(qa_pairs)} QA pairs for {llm_type.value} - {difficulty.value}")
    ...
    def process_combination(self, llm_type: LLMType, difficulty: Difficulty, num_questions: int) -> None:
        ...
        try:
            ...
            logging.error(f"Failed to process {llm_type.value} - {difficulty.value}: {str(e)}")
            raise
...
@click.option(
    ...
    default="knowledge/slamon1987.pdf",
...
def main(paper_path: str, num_questions: int):
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            futures = [
                executor.submit(generator.process_combination, llm_type, difficulty, num_questions)
            ...
            for future in as_completed(futures):
                completed += 1
                logging.info(f"Progress: {completed}/{len(combinations)} combinations completed")
                try:
                    future.result()
                except Exception as e:
                    logging.error(f"Combination failed: {str(e)}")

        logging.info(f"Dataset generation completed. Results saved in: {generator.output_file}")
